...
BENCHMARK_DATA_DIR = "./bmdb_database"
...
def pert_signal_consistency_metric(
    ...
    """
    if len(arr) < 2:
        return (None, None) #return np.nan if len(sorted_null) == 0 else (np.nan, np.nan)
    ...
    if sorted_null.ndim > 1:
        sorted_null = sorted_null.flatten()
...
def pert_signal_consistency_benchmark(
    ...
    null_results = Parallel(n_jobs=n_jobs, verbose=5)(
    ...
    # Filter out None results and sort the valid results
    null_dist = [result[0] for result in null_results if result[0] is not None]
    if null_dist:
...
def compute_best_possible_recall(
    null_distribution: np.ndarray,
    query_distribution: np.ndarray,
    recall_threshold_pairs: list
) -> dict:
    """Compute the best possible recall at given percentage thresholds for a query distribution with respect to a null distribution.
    Each recall threshold is a pair of floats (left, right) where left and right are floats between 0 and 1.
    ...
    for left_threshold, right_threshold in recall_threshold_pairs:
        # Calculate indices in the null distribution where the threshold is crossed
        left_index = int(left_threshold * len(sorted_null_distribution))
        right_index = int(right_threshold * len(sorted_null_distribution))

        # Define the range of similarity scores considered 'good'
        low_score = sorted_null_distribution[left_index]
        high_score = sorted_null_distribution[right_index]

        # Count how many query scores fall within this range
        good_recalls = np.sum((query_distribution >= low_score) & (query_distribution <= high_score))

        # Calculate recall as the proportion of good recalls to total possible good entities
        metrics[f'recall_{left_threshold}_{right_threshold}'] = good_recalls / len(query_distribution)
...
def known_relationship_benchmark(
    ...
    recall_thr_pairs: list = RECALL_PERC_THRS,
    min_req_entity_cnt: int = MIN_REQ_ENT_CNT,
    benchmark_data_dir: str = BENCHMARK_DATA_DIR,
    log_stats: bool = False,
    simulate_perfect_recall: bool = False,
) -> pd.DataFrame:
    """
    ...
    for s in benchmark_sources:
        ...
        if len(query_cossim) > 0:
            if simulate_perfect_recall :
                metrics = compute_best_possible_recall(cossim_values, query_cossim, recall_thr_pairs)
            else :
                metrics = compute_recall(cossim_values, query_cossim, recall_thr_pairs)
            metrics_lst.append(
                convert_metrics_to_df(metrics=metrics, source=s)
            )
