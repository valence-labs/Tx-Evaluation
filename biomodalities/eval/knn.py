...
class WeightedKNNClassifier(Metric):
    ...
    def _extract_labels(self, labels):
        """Converts categorical labels to indices using the pre-defined mapping."""
    ...
    def update(
        ...
        # Convert labels if they are not indices
        if train_targets is not None:
            if isinstance(train_targets.iloc[0], str) :
                train_targets = self._extract_labels(train_targets)

        if test_targets is not None:
            if isinstance(test_targets.iloc[0], str) :
                test_targets = self._extract_labels(test_targets)
    ...
    def compute(self) -> Tuple[float]:
        ...
        if self.use_pca:
            ...
            print(f"Sum of explained variance for the top 32 components: {sum(pca.explained_variance_ratio_[:64]) * 100:.2f}%")
            test_features = pca.transform(test_features)
            test_features = torch.tensor(test_features)

        # NOTE : Made this modification/comment when trying 
        # to solve out of range error for data in retrieveed neighbors.
        # Not sure yet it works or not.
        ...
        for idx in tqdm.tqdm(range(0, num_test_images, chunk_size)):
...
if __name__ == "__main__":
    ...
    knn = WeightedKNNClassifier(
        unique_labels=[str(i) for i in range(10)],  # Assuming labels are '0', '1', ..., '9'
        k=5, 
        T=0.07, 
        distance_fx="cosine",
        use_pca=True,
    ...
    train_features = torch.rand(100, 128)  # Example features
    train_labels = [str(i % 10) for i in range(100)]  # Example categorical labels

    test_features = torch.rand(30, 128)
    test_labels = [str(i % 10) for i in range(30)]

    # Update the classifier with training and testing data
    knn.update(train_features=train_features, train_targets=train_labels)
    knn.update(test_features=test_features, test_targets=test_labels)

    # Compute and print results
    top1_acc, top5_acc = knn.compute()
    print(f"k-NN accuracy @1: {top1_acc:.2f}%")
    print(f"k-NN accuracy @5: {top5_acc:.2f}%")